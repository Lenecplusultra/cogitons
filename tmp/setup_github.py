...
LABELS = [
    ...
    ("good first issue","7057ff", "Suitable for a new contributor with minimal context"),
]
...
def create_labels():
    ...
    for name, color, desc in LABELS:
        r = post(f"/repos/{REPO}/labels", {"name": name, "color": color, "description": desc})
...
MILESTONES = [
    ...
    ("Phase 1 — Authentication & Users",    "2026-04-26", "Users can create accounts, verify email, log in, reset password"),
    ("Phase 2 — Subjects",                  "2026-05-03", "Users can browse subjects. Admins can create and manage subjects."),
    ("Phase 3 — Discussions",               "2026-05-10", "Core product works — users can create, read, sort, and delete discussions"),
    ("Phase 4 — Responses",                 "2026-05-14", "Users can reply to discussions, edit and delete their own responses"),
    ("Phase 5 — Useful Votes",              "2026-05-16", "Users can mark discussions and responses as Useful — quality signal active"),
    ("Phase 6 — Search & Feed",             "2026-05-18", "Users can search the platform and see a home feed of recent discussions"),
    ("Phase 7 — Reports & Moderation",      "2026-05-22", "Users can report content. Admins can take full moderation actions."),
    ("Phase 8 — Polish & Launch",           "2026-05-30", "Platform is tested, seeded, deployed, and open to first 100 users"),
]
...
def create_milestones():
    ...
    for title, due, desc in MILESTONES:
        r = post(f"/repos/{REPO}/milestones", {
            "title": title,
...
ISSUES = {
    "Phase 0 — Foundation": [
        ("Set up monorepo folder structure (backend/, frontend/, docs/)",       ["devops", "documentation"]),
        ...
        ("Add .env.example files for backend and frontend",                      ["devops", "documentation"]),
    ...
    "Phase 1 — Authentication & Users": [
        ...
        ("Implement JWT access + refresh token issuance",                                   ["backend", "auth", "security"]),
        ("Implement POST /api/v1/auth/signup",                                              ["backend", "auth", "feature"]),
        ("Implement POST /api/v1/auth/verify-email",                                        ["backend", "auth", "feature"]),
        ("Implement POST /api/v1/auth/resend-verification",                                 ["backend", "auth", "feature"]),
        ...
        ("Implement POST /api/v1/auth/password-reset/request",                             ["backend", "auth", "feature"]),
        ("Implement POST /api/v1/auth/password-reset/confirm",                             ["backend", "auth", "feature"]),
        ...
        ("Configure Resend email integration (verification + reset)",                       ["backend", "auth"]),
        ...
        ("Implement auth state management (user context / cookies)",                        ["frontend", "auth"]),
    ],
    "Phase 2 — Subjects": [
        ...
        ("Implement PATCH /api/v1/subjects/{id} with slug history transaction",                     ["backend", "feature", "admin"]),
        ("Implement PATCH /api/v1/subjects/{id}/status (admin)",                                    ["backend", "feature", "admin"]),
        ("Build subjects list page",                                                                ["frontend", "feature"]),
        ("Build subject detail page",                                                               ["frontend", "feature"]),
        ("Build admin subject management page (create, edit, archive)",                             ["frontend", "admin", "feature"]),
    ],
    "Phase 3 — Discussions": [
        ("Create discussions SQLAlchemy model",                                                                     ["backend", "database"]),
        ("Implement GET /api/v1/subjects/{slug}/discussions (paginated, sort: recent | most_useful)",               ["backend", "feature"]),
        ("Implement POST /api/v1/discussions",                                                                      ["backend", "feature"]),
        ("Implement GET /api/v1/discussions/{id}",                                                                  ["backend", "feature"]),
        ("Implement PATCH /api/v1/discussions/{id} (owner only)",                                                   ["backend", "feature"]),
        ("Implement DELETE /api/v1/discussions/{id} with response cascade (soft delete)",                           ["backend", "feature"]),
        ("Build create discussion page (title + body form)",                                                        ["frontend", "feature"]),
        ("Build discussion detail page (body, author, timestamp, edited indicator)",                                ["frontend", "feature"]),
        ("Build discussion edit and delete controls",                                                               ["frontend", "feature"]),
        ("Build Recent / Most Useful sort controls on subject page",                                                ["frontend", "feature"]),
        ("Build numbered pagination controls",                                                                      ["frontend", "feature"]),
    ],
    "Phase 4 — Responses": [
        ("Create responses SQLAlchemy model (flat — no parent_id)",             ["backend", "database"]),
        ("Implement GET /api/v1/discussions/{id}/responses (paginated)",         ["backend", "feature"]),
        ...
        ("Implement DELETE /api/v1/responses/{id} (soft delete)",                ["backend", "feature"]),
        ...
        ("Build locked discussion UI state (no reply box, locked indicator)",    ["frontend", "feature"]),
    ],
    "Phase 5 — Useful Votes": [
        ("Create votes SQLAlchemy model with unique composite constraint",                   ["backend", "database"]),
        ("Implement POST /api/v1/votes/toggle (create or delete vote row)",                  ["backend", "feature"]),
        ("Add Useful button to discussion cards (subject page, search results)",             ["frontend", "feature"]),
        ...
        ("Update vote count in UI after toggle (optimistic update)",                         ["frontend", "feature"]),
        ("Handle visitor Useful click — trigger login prompt (Flow F-16)",                   ["frontend", "feature"]),
    ],
    "Phase 6 — Search & Feed": [
        ("Implement GET /api/v1/search (ILIKE across subjects + discussions)",               ["backend", "feature"]),
        ("Implement GET /api/v1/feed (featured subjects + recent discussions)",              ["backend", "feature"]),
        ("Build global search bar in navigation",                                            ["frontend", "feature"]),
        ("Build search results page (subjects first, discussions second)",                   ["frontend", "feature"]),
        ("Build empty search result state with browse subjects link",                        ["frontend", "feature"]),
    ...
    "Phase 7 — Reports & Moderation": [
        ...
        ("Create moderation_logs SQLAlchemy model",                                                  ["backend", "database", "moderation"]),
        ("Implement POST /api/v1/reports",                                                           ["backend", "feature", "moderation"]),
        ("Implement GET /api/v1/admin/reports (paginated queue)",                                    ["backend", "admin", "moderation"]),
        ("Implement POST /api/v1/admin/reports/{id}/dismiss",                                        ["backend", "admin", "moderation"]),
        ("Implement POST /api/v1/admin/content/remove (soft delete)",                               ["backend", "admin", "moderation"]),
        ("Implement POST /api/v1/admin/discussions/{id}/lock",                                       ["backend", "admin", "moderation"]),
        ("Implement POST /api/v1/admin/discussions/{id}/unlock",                                     ["backend", "admin", "moderation"]),
        ("Implement POST /api/v1/admin/users/{id}/suspend",                                          ["backend", "admin", "moderation"]),
        ("Implement POST /api/v1/admin/users/{id}/restore",                                          ["backend", "admin", "moderation"]),
        ("Ensure ModerationLog entry is written on every admin action",                              ["backend", "moderation", "testing"]),
        ("Build report modal with reason field",                                                     ["frontend", "feature", "moderation"]),
        ("Build admin reports queue page",                                                           ["frontend", "admin", "moderation"]),
        ("Build moderation action controls (dismiss, remove, lock, suspend)",                        ["frontend", "admin", "moderation"]),
        ("Build locked discussion indicator in thread view",                                         ["frontend", "moderation"]),
    ...
    "Phase 8 — Polish & Launch": [
        ...
        ("Write backend integration tests for discussion and response endpoints",            ["testing"]),
        ("Write backend integration tests for vote, search, and feed endpoints",            ["testing"]),
        ("Write backend integration tests for report and admin endpoints",                   ["testing", "moderation"]),
        ...
        ("Verify rate limiting is active on all auth endpoints",                             ["security", "backend"]),
        ("Verify all soft-delete behavior across discussions and responses",                 ["testing", "backend"]),
        ("Verify moderation logs are written for all admin actions",                         ["testing", "moderation"]),
        ("Verify input validation and sanitization on all POST/PATCH endpoints",             ["testing", "security"]),
        ("Run full responsive design check on mobile browser",                               ["frontend", "testing"]),
        ("Audit and fix loading states and error states on all pages",                       ["frontend", "feature"]),
        ("Seed minimum 20 subjects in production database",                                  ["devops", "documentation"]),
        ("Seed minimum 50 discussions across top subjects",                                  ["devops", "documentation"]),
        ("Connect production custom domain to Vercel",                                       ["devops"]),
        ("Confirm SSL certificate is active on production domain",                           ["devops", "security"]),
...
if __name__ == "__main__":
    ...
    print(f"Repo: {REPO}")
    print(f"Labels: {len(LABELS)} | Milestones: {len(MILESTONES)} | Issue groups: {len(ISSUES)}")
    create_labels()
